...
export function buildPedestrianPath(world: GameWorld, from: Vector2, to: Vector2,
  searchRadius: number = CROSSINGS_CONFIG.searchRadius): readonly PedestrianWaypoint[] {
  if (!Number.isFinite(searchRadius) || searchRadius < 0) throw new RangeError('Invalid crossing search radius')
  const used = new Set<string>()

  function segment(start: Vector2, end: Vector2): PedestrianWaypoint[] {
    const intersections = [...world.roads.values()].flatMap((road) => {
    ...
    for (const contact of intersections) {
      const candidates = [...world.crossings.values()]
        ...
        .map((crossing) => ({ crossing, distance: distanceBetween(crossing.position, contact.position) }))
...
function intersectRoad(from: Vector2, to: Vector2, road: Road) {
  ...
  const side = (point: Vector2) => (point.x - road.position.x) * normal.x + (point.y - road.position.y) * normal.y
...
export function pedestrianPathDistance(path: readonly Vector2[]): number {
  let distance = 0
  for (let index = 1; index < path.length; index++) distance += distanceBetween(path[index - 1]!, path[index]!)
  return distance
}
